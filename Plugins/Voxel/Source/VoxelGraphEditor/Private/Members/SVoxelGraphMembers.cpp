...
#include "Members/VoxelMembersSchemaAction_EditorGraph.h"
#include "Members/VoxelMembersSchemaAction_Function.h"
#include "Members/VoxelMembersSchemaAction_Parameter.h"
#include "Members/VoxelMembersSchemaAction_FunctionInput.h"
#include "Members/VoxelMembersSchemaAction_FunctionOutput.h"
...
void SVoxelGraphMembers::Construct(const FArguments& Args)
{
	...
	CommandList->MapAction(FGenericCommands::Get().Paste, MakeWeakPtrDelegate(this, [this]
	{
		...
		{
			FVoxelMembersSchemaAction_FunctionInput::OnPaste(*Toolkit, *TerminalGraph, ClipboardText, GetPasteCategory());
		}
		break;
		case EVoxelGraphMemberSection::FunctionOutputs:
		{
			FVoxelMembersSchemaAction_FunctionOutput::OnPaste(*Toolkit, *TerminalGraph, ClipboardText, GetPasteCategory());
		}
		break;
	...
	[
		...
		[
			...
			.OnCollectStaticSections_Lambda([=, this](TArray<int32>& StaticSectionIds)
			{
				const TSharedPtr<FVoxelGraphToolkit> Toolkit = GetToolkit();
				if (!ensure(Toolkit))
				{
				...
				{
					StaticSectionIds.Add(GetSectionId(EVoxelGraphMemberSection::Graphs));
					StaticSectionIds.Add(GetSectionId(EVoxelGraphMemberSection::Functions));
				}
				...
				if (const UVoxelTerminalGraph* TerminalGraph = GetTerminalGraph())
				{
					if (TerminalGraph->IsFunction())
					{
						...
						StaticSectionIds.Add(GetSectionId(EVoxelGraphMemberSection::FunctionOutputs));
					}
				}
			...
			.OnGetSectionTitle_Lambda([=, this](const int32 SectionId) -> FText
			{
				switch (GetSection(SectionId))
				{
				default: ensure(false); return {};
				case EVoxelGraphMemberSection::Graphs: return INVTEXT("Graphs");
				case EVoxelGraphMemberSection::Functions:
				{
					const TSharedPtr<FVoxelGraphToolkit> Toolkit = GetToolkit();
					if (!ensure(Toolkit))
					...
					for (const FGuid& Guid : Graph.GetTerminalGraphs())
					{
						if (Guid != GVoxelMainTerminalGraphGuid &&
							!Graph.FindTerminalGraph_NoInheritance(Guid))
						{
							NumOverridable++;
						}
					}
				...
				case EVoxelGraphMemberSection::FunctionOutputs: return INVTEXT("Function Outputs");
				case EVoxelGraphMemberSection::LocalVariables: return INVTEXT("Local Variables");
				}
			})
			.OnGetSectionWidget_Lambda([=, this](TSharedRef<SWidget> RowWidget, const int32 SectionId) -> TSharedRef<SWidget>
			{
				...
				case EVoxelGraphMemberSection::Graphs: return SNullWidget::NullWidget;
				case EVoxelGraphMemberSection::Functions:
				{
				...
				case EVoxelGraphMemberSection::FunctionOutputs: return CreateAddButton(INVTEXT("Add new output"));
						...
						if (TerminalGraph->IsFunction())
						{
							...
							MenuBuilder.AddMenuEntry(
								...
								FUIAction
								{
									MakeWeakPtrDelegate(this, [=, this]
									{
										OnAddNewMember(GetSectionId(EVoxelGraphMemberSection::FunctionOutputs));
									})
								});
						}
			...
			.OnCollectAllActions_Lambda([=, this](FGraphActionListBuilderBase& OutAllActions)
			{
			...
			.OnCreateWidgetForAction_Lambda([](const FCreateWidgetForActionData* CreateData)
			{
				return
					...
					.VAlign(VAlign_Center)
					[
						static_cast<FVoxelMembersSchemaAction_Base&>(*CreateData->Action).CreatePaletteWidget(*CreateData)
					];
			})
			.OnCanRenameSelectedAction_Lambda([](const TWeakPtr<FGraphActionNode> WeakActionNode)
			{
				...
				if (ActionNode->GetPrimaryAction() &&
					!ActionNode->GetPrimaryAction()->CanBeRenamed())
				{
					return false;
				}
...
void SVoxelGraphMembers::Refresh()
{
...

	{
		...
		MembersMenu->RefreshAllActions(true, false);
	}
...
void SVoxelGraphMembers::SetTerminalGraph(UVoxelTerminalGraph* NewTerminalGraph)
{
	const TSharedPtr<FVoxelGraphToolkit> Toolkit = GetToolkit();
	if (!ensure(Toolkit))
	{
...
TArray<TSharedRef<FVoxelMembersSchemaAction_Base>> SVoxelGraphMembers::GetActions()
{
	...
	INLINE_LAMBDA
	{
		if (Graph.IsFunctionLibrary())
		{
			...
			for (const FGuid& Guid : Graph.GetTerminalGraphs())
			{
				...
				const TSharedRef<FVoxelMembersSchemaAction_LibraryFunction> Action = MakeShared<FVoxelMembersSchemaAction_LibraryFunction>(
		...
		{
			const TSharedRef<FVoxelMembersSchemaAction_MainGraph> Action = MakeShared<FVoxelMembersSchemaAction_MainGraph>(
				INVTEXT(""),
				INVTEXT("Main Graph"),
				INVTEXT(""),
				2,
				INVTEXT(""),
				GetSectionId(EVoxelGraphMemberSection::Graphs));
		...
		{
			const TSharedRef<FVoxelMembersSchemaAction_EditorGraph> Action = MakeShared<FVoxelMembersSchemaAction_EditorGraph>(
				INVTEXT(""),
				INVTEXT("Editor Graph"),
				INVTEXT(""),
				2,
				INVTEXT(""),
				GetSectionId(EVoxelGraphMemberSection::Graphs));

			Action->MemberGuid = GVoxelEditorTerminalGraphGuid;
			Action->WeakToolkit = Toolkit;
			Action->WeakMembers = SharedThis(this);
			Action->WeakGraph = &Graph;
		...
		for (const FGuid& Guid : Graph.GetTerminalGraphs())
		{
			...
			const TSharedRef<FVoxelMembersSchemaAction_Function> Action = MakeShared<FVoxelMembersSchemaAction_Function>(
				FText::FromString(Metadata.Category),
				FText::FromString(Metadata.DisplayName),
				FText::FromString(Description),
				2,
	...
	for (const FGuid& Guid : Graph.GetParameters())
	{
		...
		const TSharedRef<FVoxelMembersSchemaAction_Parameter> Action = MakeShared<FVoxelMembersSchemaAction_Parameter>(
	...
	if (TerminalGraph->IsFunction())
	{
		...
		{
			const FVoxelGraphFunctionInput& Input = TerminalGraph->FindInputChecked(Guid);

			const TSharedRef<FVoxelMembersSchemaAction_FunctionInput> Action = MakeShared<FVoxelMembersSchemaAction_FunctionInput>(
		...
		{
			const FVoxelGraphFunctionOutput& Output = TerminalGraph->FindOutputChecked(Guid);

			const TSharedRef<FVoxelMembersSchemaAction_FunctionOutput> Action = MakeShared<FVoxelMembersSchemaAction_FunctionOutput>(
				...
				GetSectionId(EVoxelGraphMemberSection::FunctionOutputs));
	...
	for (const FGuid& Guid : TerminalGraph->GetLocalVariables())
	{
		...
		const TSharedRef<FVoxelMembersSchemaAction_LocalVariable> Action = MakeShared<FVoxelMembersSchemaAction_LocalVariable>(
...
void SVoxelGraphMembers::OnAddNewMember(const int32 SectionId)
{
	...
	const UE_506_SWITCH(FVector2D, FVector2f) GraphLocation(Location);
	switch (GetSection(SectionId))
	{
	default:
	{
	...
	case EVoxelGraphMemberSection::Functions:
	{
		...
		Action.PerformAction(EdGraph, nullptr, GraphLocation);
	}
	break;
	case EVoxelGraphMemberSection::Parameters:
	{
...
void SVoxelGraphMembers::SetupTerminalGraphOverride(UVoxelTerminalGraph& TerminalGraph)
{
	...
	UVoxelGraphNode* CallParentNode = INLINE_LAMBDA
	{
		...
		return Action.Apply(EdGraph, UE_506_SWITCH(FVector2D, FVector2f)::ZeroVector);
	};
	...
	for (const FGuid& Guid : TerminalGraph.GetFunctionInputs())
	{
		...
		const UVoxelGraphNode* InputNode = Action.Apply(EdGraph, UE_506_SWITCH(FVector2D, FVector2f)(-200.f, InputIndex * 75.f));
		if (!ensure(InputNode))
		{
			continue;
		}
	...
	for (const FGuid& Guid : TerminalGraph.GetFunctionOutputs())
	{
		...
		const UVoxelGraphNode* OutputNode = Action.Apply(EdGraph, UE_506_SWITCH(FVector2D, FVector2f)(300.f, OutputIndex * 75.f));
