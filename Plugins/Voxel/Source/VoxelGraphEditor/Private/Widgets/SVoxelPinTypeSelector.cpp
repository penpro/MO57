...
void SVoxelPinTypeSelector::FillTypesList()
{
	...
	AllowedTypes.Get().Iterate([&](const FVoxelPinType& Type)
	{
		...
		if (ExposedType.Is<uint8>() &&
			ExposedType.GetEnum())
		{
			if (ExposedType.IsUserDefinedEnum())
			{
				TargetCategory = "User Enums";
			}
			else
			{
		...
		else if (ExposedType.IsStruct())
		{
			...
			else if (ExposedType.IsUserDefinedStruct())
			{
				TargetCategory = "User Structs";
			}
			else if (
				...
				!ExposedType.Is<FVoxelDoubleLinearColor>())
			{
