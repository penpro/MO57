...
{
	AllowedClasses = TVoxelSet<const UClass*>(InArgs._AllowedClasses).Array();
	AllowedClasses.Remove(nullptr);
	...
	{
		TVoxelOptional<bool> IsActor;
		for (const UClass* Class : AllowedClasses)
		{
			const bool bNewIsActor = Class->IsChildOf(AActor::StaticClass());
			ensure(!IsActor.IsSet() || IsActor == bNewIsActor);
			IsActor = bNewIsActor;
		}

		bIsActor = IsActor.IsSet() && IsActor.GetValue();
	}
...
bool SVoxelGraphObjectSelector::OnAssetDraggedOver(const TArrayView<FAssetData> InAssets, FText& OutReason) const
{
	...
	if (!AssetObject ||
		!IsClassAllowed(AssetObject->GetClass()))
	{
		return false;
	}
...
void SVoxelGraphObjectSelector::OnUseClicked() const
{
	...
	for (const UClass* Class : AllowedClasses)
	{
		...
		{
			if (const UObject* Selection = GEditor->GetSelectedActors()->GetTop(Class))
			{
				SetValue(Selection);
				return;
			}
		...
		{
			if (const UObject* Selection = GEditor->GetSelectedObjects()->GetTop(Class))
			{
				SetValue(Selection);
				return;
			}
...
void SVoxelGraphObjectSelector::SetValue(const FAssetData& AssetData) const
{
	...
	{
		const TSharedRef<FVoxelNotification> Notification = FVoxelNotification::Create(AssetReferenceFilterFailureReason.ToString());
		Notification->ExpireAndFadeoutIn(4.f);
		return;
	}
...
bool SVoxelGraphObjectSelector::CanSetBasedOnAssetReferenceFilter(const FAssetData& InAssetData, FText& OutOptionalFailureReason) const
{
	...
	for (const FAssetData& AssetData : OwnerAssetDataArray)
	{
		if (AssetData.IsValid())
		{
			AssetReferenceFilterContext.AddReferencingAsset(AssetData);
		}
	}
...
bool SVoxelGraphObjectSelector::IsClassAllowed(const UClass* Class) const
{
	for (const UClass* AllowedClass : AllowedClasses)
	{
		if (Class->IsChildOf(AllowedClass))
		{
			return true;
		}

		if (AllowedClass->HasAnyClassFlags(CLASS_Interface) &&
			Class->ImplementsInterface(AllowedClass))
		{
			return true;
		}
	}
