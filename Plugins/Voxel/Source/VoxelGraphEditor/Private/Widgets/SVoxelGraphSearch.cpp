...
bool FVoxelGraphSearchSettings::ShowTag(const EVoxelGraphSearchResultTag Tag) const
{
	...
	case EVoxelGraphSearchResultTag::FunctionInput: return bSearchParameters;
	case EVoxelGraphSearchResultTag::FunctionOutput: return bSearchParameters;
...
void SVoxelGraphSearch::MakeSearchQuery(const FString& SearchQuery)
{
	...
	const TSharedRef<FVoxelGraphSearchItem::FContext> Context = MakeShared<FVoxelGraphSearchItem::FContext>();
	...
	if (Tokens.Num() > 0)
	{
		...
		if (bIsGlobalSearch)
		{
			...
			{
				const TSharedRef<FVoxelGraphSearchItem_Graph> Item = MakeShared<FVoxelGraphSearchItem_Graph>(&Graph);
		...
		else
		{
			...
			const TSharedRef<FVoxelGraphSearchItem_Graph> Item = MakeShared<FVoxelGraphSearchItem_Graph>(Toolkit->Asset);
			Item->Initialize(Context);
			RootItems.Add(Item);
		}
	}
	...
	if (RootItems.Num() == 0)
	{
		RootItems.Add(MakeShared<FVoxelGraphSearchItem_Text>(EVoxelGraphSearchResultTag::Special, "", "No Results found"));
	}
	...
	{
		const TSharedPtr<FVoxelGraphSearchItem> Item = QueuedItems.Pop(EAllowShrinking::No);
