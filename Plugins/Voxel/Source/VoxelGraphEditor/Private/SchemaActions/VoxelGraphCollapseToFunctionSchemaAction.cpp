...
UEdGraphNode* FVoxelGraphSchemaAction_CollapseToFunction::PerformAction(UEdGraph* ParentGraph, UEdGraphPin* FromPin, const UE_506_SWITCH(FVector2D, FVector2f&) Location, bool bSelectNewNode)
{
	...
	UVoxelTerminalGraph* NewFunctionGraph;
	{
		...
		Action.PerformAction(Toolkit->GetActiveEdGraph(), nullptr, UE_506_SWITCH(FVector2D, FVector2f)::ZeroVector);
		NewFunctionGraph = Action.OutNewFunction.Resolve();
	...
	UVoxelGraphNode_CallMemberFunction* NewFunctionNode;
	{
		FVoxelGraphSchemaAction_NewCallFunctionNode Action;
		Action.Guid = NewFunctionGraph->GetGuid();
		NewFunctionNode = Cast<UVoxelGraphNode_CallMemberFunction>(Action.PerformAction(ParentGraph, nullptr, AvgNodePosition));
	}
...
void FVoxelGraphSchemaAction_CollapseToFunction::AddLocalVariableInputs(UVoxelTerminalGraph* Graph)
{
	for (const auto& It : CopiedLocalVariables)
	{
		...
		UE_506_SWITCH(FVector2D, FVector2f) Position = InputDeclarationPosition;
		...
		UVoxelGraphNode_FunctionInput* FunctionInput;
		{
			...
			FunctionInput = Cast<UVoxelGraphNode_FunctionInput>(Action.PerformAction(&Graph->GetEdGraph(), nullptr, Position, false));
		}
...
void FVoxelGraphSchemaAction_CollapseToFunction::AddDeclarationOutputs(UVoxelTerminalGraph* Graph)
{
	for (const auto& It : CopiedLocalVariables)
	{
		...
		UE_506_SWITCH(FVector2D, FVector2f) Position = OutputDeclarationPosition;
		...
		UVoxelGraphNode_FunctionOutput* FunctionOutput;
		{
			...
			FunctionOutput = Cast<UVoxelGraphNode_FunctionOutput>(Action.PerformAction(&Graph->GetEdGraph(), LocalVariableUsage->GetOutputPin(0), Position, false));
		}
...
void FVoxelGraphSchemaAction_CollapseToFunction::AddOuterInputsOutputs(UVoxelTerminalGraph* Graph)
{
	...
	{
		if (!ensure(It.Value->OriginalNode.IsValid_Slow()) ||
			!ensure(It.Value->NewNode.IsValid_Slow()))
		{
			continue;
		}
		...
		for (auto& OuterPinIt : It.Value->OutsideConnectedPins)
		{
			...
			for (const FEdGraphPinReference& WeakOuterPin : OuterPinIt.Value)
			{
				...
				{
					UE_506_SWITCH(FVector2D, FVector2f) Position(NewNode->NodePosX, NewNode->NodePosY - 200.f);
					if (NewPin->Direction == EGPD_Input)
					{
						FVoxelGraphSchemaAction_NewFunctionInput Action;
						...
						const UVoxelGraphNode_FunctionInput* InputNode = Cast<UVoxelGraphNode_FunctionInput>(Action.PerformAction(&Graph->GetEdGraph(), NewPin, Position, false));
						if (ensure(InputNode))
						{
					...
					else
					{
						...
						const UVoxelGraphNode_FunctionOutput* OutputNode = Cast<UVoxelGraphNode_FunctionOutput>(Action.PerformAction(&Graph->GetEdGraph(), NewPin, Position, false));
...
void FVoxelGraphSchemaAction_CollapseToFunction::FixupMainGraph(const UVoxelGraphNode_CallMemberFunction* FunctionNode, UEdGraph* EdGraph)
{
	...
	// Link parameters/local variables
	{
		for (const auto& It : CopiedLocalVariables)
		{
			if (It.Value->InputGuid.IsValid())
			{
				...
				UE_506_SWITCH(FVector2D, FVector2f) Position(AvgNodePosition.X - 200.f, AvgNodePosition.Y);
...
void FVoxelGraphSchemaAction_CollapseToFunction::ImportNodes(UVoxelTerminalGraph* Graph, const FString& ExportText)
{
	...
	AvgNodePosition = UE_506_SWITCH(FVector2D, FVector2f)::ZeroVector;
	...
	{
		const UE_506_SWITCH(FVector2D, FVector2f) NodePosition = GetNodePosition(Node);
...
UE_506_SWITCH(FVector2D, FVector2f) FVoxelGraphSchemaAction_CollapseToFunction::GetNodePosition(const UEdGraphNode* Node) const
{
	return UE_506_SWITCH(FVector2D, FVector2f)(Node->NodePosX - AvgNodePosition.X, Node->NodePosY - AvgNodePosition.Y);
}