...
TSharedRef<SWidget> SVoxelCurveEditor::MakeTemplatesMenu()
{
	...
	for (const FVoxelInlineCurveTemplate& CurveTemplate : GetDefault<UVoxelSettings>()->CurveTemplates)
	{
		...
		const FText CurveDisplayName = CurveTemplate.DisplayNameOverride.IsEmpty()
		...
		MenuBuilder.AddWidget(
			...
			.ToolTipText(FText::FromString(CurveDisplayName.ToString() + "\nClick to apply this template to the selected curves."))
			.ContentPadding(3.f)
			.Content()
			[
				SNew(SHorizontalBox)
