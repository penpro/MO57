...
void UVoxelGraphNode_LocalVariable::AllocateDefaultPins()
{
	...
	if (ensure(TerminalGraph))
	{
		...
		GVoxelGraphTracker->OnLocalVariableChanged(*TerminalGraph).Add(FOnVoxelGraphChanged::Make(OnLocalVariableChangedPtr, this, [=, this]
		{
			ReconstructNode();
		}));
	}
...
{
	return FVoxelGraphVisuals::GetPinColor(GetLocalVariableSafe().Type);
}
