...
TSharedPtr<FVoxelGraphToolkit> FVoxelGraphSearchItem::OpenToolkit(const UVoxelGraph& Graph)
{
	...
	return CastStructEnsured<FVoxelGraphToolkit>(Toolkit->AsShared());
}
...
void FVoxelGraphSearchItem_TerminalGraph::Build()
{
	...
	if (TerminalGraph->IsFunction())
	{
		...
		{
			AddChild<FVoxelGraphSearchItem_FunctionInput>(Guid, TerminalGraph->FindInputChecked(Guid));
		}
		for (const FGuid& Guid : TerminalGraph->GetFunctionOutputs())
		{
			AddChild<FVoxelGraphSearchItem_FunctionOutput>(Guid, TerminalGraph->FindOutputChecked(Guid));
		}
	}
...
{
	FVoxelUtilities::FocusObject(WeakTerminalGraph.Resolve());
}
...
{
	const UEdGraphNode* Node = WeakNode.Resolve();
	if (!ensure(Node))
	{
		return {};
	}
...
void FVoxelGraphSearchItem_Node::Build()
{
	...
	if (const UVoxelGraphNode_CustomizeParameter* ParameterNode = Cast<UVoxelGraphNode_CustomizeParameter>(Node))
	{
		AddChild<FVoxelGraphSearchItem_Parameter>(ParameterNode->Guid, ParameterNode->GetParameterSafe());
		return;
	}
	...
	{
		AddChild<FVoxelGraphSearchItem_FunctionInput>(InputNode->Guid, InputNode->GetInputSafe());
		return;
	}
	if (const UVoxelGraphNode_FunctionOutput* OutputNode = Cast<UVoxelGraphNode_FunctionOutput>(Node))
	{
		AddChild<FVoxelGraphSearchItem_FunctionOutput>(OutputNode->Guid, OutputNode->GetOutputSafe());
		return;
	}
...
{
	FVoxelUtilities::FocusObject(WeakNode.Resolve());
}
...
void FVoxelGraphSearchItem_FunctionInput::Build()
{
	FVoxelGraphSearchItem_Property::Build();

	const FString DefaultValue = Input.DefaultPinValue.ExportToString();
