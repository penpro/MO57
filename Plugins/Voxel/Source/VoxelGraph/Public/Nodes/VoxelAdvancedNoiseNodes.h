...
enum class EVoxelAdvancedNoiseOctaveType : uint8
{
	...
	SmoothSimplex = 6 UMETA(ToolTip = "Simplex2D"),
	BillowySimplex = 7 UMETA(ToolTip = "abs(Simplex2D) * 2 - 1"),
	RidgedSimplex = 8 UMETA(ToolTip = "(1 - abs(Simplex2D)) * 2 - 1"),

	SmoothValue = 9 UMETA(ToolTip = "Value2D"),
	BillowyValue = 10 UMETA(ToolTip = "abs(Value2D) * 2 - 1"),
	RidgedValue = 11 UMETA(ToolTip = "(1 - abs(Value2D)) * 2 - 1"),
};

// Generates multi-octave, highly configurable height noise for easy procedural creation
USTRUCT(Category = "Noise")
struct VOXELGRAPH_API FVoxelNode_AdvancedNoise2D : public FVoxelNode
{
	GENERATED_BODY()
...
// Generates multi-octave, highly configurable height noise for easy procedural creation
USTRUCT(Category = "Noise")
struct VOXELGRAPH_API FVoxelNode_AdvancedNoise3D : public FVoxelNode
{
	GENERATED_BODY()
	GENERATED_VOXEL_NODE_BODY()
