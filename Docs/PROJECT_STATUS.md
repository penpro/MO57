...
> - **STILL OPEN:** **H25** (HIGH — craft pause/resume + save/load wiped partial progress to ~0; StartCrafting re-anchored at UtcNow with no back-date → next tick recomputes ~0 and overwrites Progress. **FIXED July 8** by back-dating the anchor by the front entry's Progress) · **H23** (HIGH, effort-L, design-y — single world-global FMOHarvestContext makes concurrent player+pawn harvests cancel each other, and the PLAYER completion clock is still widget-owned; per-harvester context + move the clock to the subsystem) · **H17** (MEDIUM residual — terraform ServerApplyTerraform guards authority/reach/height but has no rate/duration limit → modified-client instant-crater spam + SweepModifiedZones host DoS) · **C7** (MEDIUM residual — possession Server RPCs clamp spatial params but forward a client-supplied TSubclassOf verbatim to SpawnActorDeferred; needs a class whitelist) · **C8** (mostly closed — ValidateInstanceHarvestRequest gates bounds/reach/rate; only LOS tuning left).
