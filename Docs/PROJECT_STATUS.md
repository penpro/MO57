...
> - **H29 — LATENT (fixed July 7 as a contract correction, symptom NOT currently reachable):** SaveWorldToSlot never advanced LoadedWorldSave after a write, violating the header's "in-memory copy of save state" contract. BUT the audit's (and even the adversarial pass's) player-facing scenarios are foreclosed: `HandleIdentityRegistered` binds `OnOwnerDestroyedWithGuid` on EVERY identity actor (pawns + creatures), so any in-session despawn — including creature FIFO `DespawnOldest` — records the GUID in the destroyed ledger and the merge correctly excludes it. There is no in-session unload-without-record path, so despawned pawns are neither lost nor reverted. Fixed anyway (`LoadedWorldSave = SaveObject; SessionPlayTimeSeconds = 0;` at the save chokepoint) to honor the contract and pre-empt the bug going live if real level streaming (suppress-record unload) is ever added. **Lesson: verify agent reachability claims to the destroy/record layer — two audit stages both assumed a stream-out path the destroyed-GUID binding actually closes.**
> - **STILL OPEN:** **H36** (CONFIRMED — crafting queue fast-forwards by real wall-clock offline time, 7-day clamp, the only pawn system that does; Principle-11 violation. **Design fork:** kill offline crafting vs. make offline a whole-sim GameClock decision — hold for Wes) · **H39 residual** — station-targeted jobs reloaded with null TargetActor/StorageActor weak ptrs (GUIDs survived, weak ptrs don't serialize); **FIXED July 7** via ResolveJobActorRefs GUID→actor rehydration in the job queue.
...
> **Harvest + MP-authority + security/GC clusters re-verification — July 8, 2026** (17-agent workflow, adversarial verify). Same staleness pattern — most were fixed after June 11:
> - **ALREADY FIXED / REFUTED:** **C4** (audio async UAF — CreateWeakLambda + handle cancel) · **C5** (mod recipe overlay GC-safe) — *both CRITICALs closed; the "9 criticals" headline is stale* · **H18** (combat verbs have client→server transport + authority guards) · **H19** (ServerPlaceBuilding) · **H20** (crafting-queue mutators/tick authority-guarded) · **H22** (foraging timed-action, tracked defect) · **H24** (pawn harvest reads BaseActionTime).
> - **STILL OPEN:** **H25** (HIGH — craft pause/resume + save/load wiped partial progress to ~0; StartCrafting re-anchored at UtcNow with no back-date → next tick recomputes ~0 and overwrites Progress. **FIXED July 8** by back-dating the anchor by the front entry's Progress) · **H23** (HIGH — **concurrency CLOBBER FIXED July 13** (`9c7178c6`): per-harvester `TMap<TWeakObjectPtr<AActor>,FMOHarvestContext>` so player + N survivors / co-op harvest simultaneously; BeginHarvest cancels only the caller's own prior harvest; interrupts route per-harvester via `Context.AffectedActor`; gate `test_harvest_concurrent.py` 9/9. Residual **by design**: the player completion clock stays widget-driven, but the subsystem authoritatively **vetoes** early/forged completion via the wall-clock duration check — so the clock ownership is no longer a correctness hole, just a division of labor) · **H17** (MEDIUM residual — terraform ServerApplyTerraform guards authority/reach/height but has no rate/duration limit → modified-client instant-crater spam + SweepModifiedZones host DoS) · **C7** (MEDIUM residual — possession Server RPCs clamp spatial params but forward a client-supplied TSubclassOf verbatim to SpawnActorDeferred; needs a class whitelist) · **C8** (mostly closed — ValidateInstanceHarvestRequest gates bounds/reach/rate; only LOS tuning left).
> - New authority residuals to ticket: H19 ServerPlaceBuilding trusts the client FTransform (no reach/collision recheck); H22 foraging spawn+XP runs client-side in a UI widget with no authority guard. The H17/C7/C8 family wants a 2-client PIE / mptest transport harness to exercise the trust boundary.

### CRITICAL — Architectural

| Code | Issue | Location | Status (Jun 11) |
|------|-------|----------|--------|
| C1 | **Monolithic runtime module** | `MOFramework.Build.cs` | **IN PROGRESS — phases 1-3 landed 2026-07-06/07** (phase 3: MOFrameworkMedical module — anatomy/vitals/metabolism/mental/adrenaline/medical subsystem; clock descended to Core behind UMOClockSaveDomainAdapter; ambient environment + skill-level + insulation contracts in Core). Was phase-1 note:: `MOFrameworkCore` module carved (18 headers + 3 cpps: DataTable row schemas, *Types.h families, cross-system interfaces, UI delegate library, blackboard keys). Campaign order for later phases: (2) Medical — needs owner-pointer decoupling in FMOWoundList-style FastArrays first; (3) Inventory/Crafting — needs FMOCraftResult out of the subsystem header; (4) UI — biggest, needs the UIManager↔widget seam. CARVE RULE learned the hard way: include analysis is NOT sufficient — forward-declared UCLASS refs in UPROPERTY/UFUNCTION positions are hard UHT deps; sweep `[UAF]MO\w+` tokens in UHT-visible positions before moving, then let the compiler judge. 9 candidates bounced back this way (interfaces returning component ptrs, types with replication owner-pointers, recipe row's BuildableActorClass). |
...
| H23 | ~~Harvest simulation clock owned by the UI — completion decided by widget wall-clock NativeTick; `CompleteHarvest` performs zero duration validation; the subsystem's own timer is dead code; and a single world-global FMOHarvestContext means concurrent harvests (co-op, player+pawn) cancel each other.~~ **FIXED (concurrency July 13 `9c7178c6`; duration validation June 30):** per-harvester context map + authoritative wall-clock veto in CompleteHarvest. Gate `test_harvest_concurrent.py` 9/9. | `MOHarvestSubsystem.cpp` (ContextsByHarvester) |
