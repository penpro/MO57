...
|------|-------|----------|--------|
| C1 | **Monolithic runtime module** | `MOFramework.Build.cs` | **IN PROGRESS — phase 1 landed 2026-07-06**: `MOFrameworkCore` module carved (18 headers + 3 cpps: DataTable row schemas, *Types.h families, cross-system interfaces, UI delegate library, blackboard keys). Campaign order for later phases: (2) Medical — needs owner-pointer decoupling in FMOWoundList-style FastArrays first; (3) Inventory/Crafting — needs FMOCraftResult out of the subsystem header; (4) UI — biggest, needs the UIManager↔widget seam. CARVE RULE learned the hard way: include analysis is NOT sufficient — forward-declared UCLASS refs in UPROPERTY/UFUNCTION positions are hard UHT deps; sweep `[UAF]MO\w+` tokens in UHT-visible positions before moving, then let the compiler judge. 9 candidates bounced back this way (interfaces returning component ptrs, types with replication owner-pointers, recipe row's BuildableActorClass). |
